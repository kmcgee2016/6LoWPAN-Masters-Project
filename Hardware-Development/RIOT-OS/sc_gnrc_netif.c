...
#else
//KMG csv fcn.
int _netif_stats(kernel_pid_t iface, unsigned module, bool reset){
	    netstats_t *stats;
	    if(module == NETSTATS_LAYER2 || reset){/*Just for craic!*/}
    int res = gnrc_netapi_get(iface, NETOPT_STATS, NETSTATS_LAYER2, &stats,
                              sizeof(&stats));
    /*L2*/
	printf("%s,"
		   "RX packets, %u, bytes, %u,"
		   "TX packets, %u, (Multicast:, %u,)  bytes, %u,"
		   "TX succeeded, %u, errors, %u,",
		   _netstats_module_to_str(NETSTATS_LAYER2),
		   (unsigned) stats->rx_count,
		   (unsigned) stats->rx_bytes,
		   (unsigned) (stats->tx_unicast_count + stats->tx_mcast_count),
		   (unsigned) stats->tx_mcast_count,
		   ...
		   (unsigned) stats->tx_failed);
	/*L3*/	
	res = gnrc_netapi_get(iface, NETOPT_STATS, NETSTATS_IPV6, &stats,
                              sizeof(&stats));    
	printf("%s,"
		   "RX packets, %u, bytes, %u,"
		   "TX packets, %u, (Multicast:, %u,)  bytes, %u,"
		   "TX succeeded, %u, errors, %u,",
		   _netstats_module_to_str(NETSTATS_IPV6),
		   (unsigned) stats->rx_count,
		   (unsigned) stats->rx_bytes,
		   (unsigned) (stats->tx_unicast_count + stats->tx_mcast_count),
		   (unsigned) stats->tx_mcast_count,
