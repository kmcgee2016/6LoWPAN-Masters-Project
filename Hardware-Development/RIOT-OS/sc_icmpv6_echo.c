...
int _handle_reply(gnrc_pktsnip_t *pkt, uint32_t time)
{
    ...
    if ((byteorder_ntohs(icmpv6_hdr->id) == id) && _expected_seq(seq)) {
        ...
        printf("%u bytes from %s: id=%" PRIu16 " seq=%" PRIu16 " hop limit=%u, time=, %"
               PRIu32 ".%03" PRIu32 " ,ms", (unsigned) icmpv6->size,
...
{
	if(sum_rtt < 2 || min_rtt < 2 || max_rtt < 2){/*Here for craic*/}
    ...
    if (success > 0) {
        ...
        //I do not need or want these stats:
        /*       
...
int _icmpv6_ping(int argc, char **argv)
{
    ...
    if (argc < 2) {
        ...
        param_offset = 1; //(KMG) used to tell if there is a count argument(need to offset the IPv6 address argument, then so!)
    }
    else {
        count = 3;
    }
