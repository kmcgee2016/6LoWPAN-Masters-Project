...
uint32_t numPackets = 1000; // packets
TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");



//sockets(smart socket pointers):
Ptr<Socket> sink = Socket::CreateSocket (n1, tid); //this is the definition of the sink on node 3(final node {0,1,2,3})
...
//node 0 //Source Node
Inet6SocketAddress remote0 = Inet6SocketAddress(deviceInterfaces.GetAddress (1,1), port); //this should be the final destination => the interface should be:
...
///PCAP traces:
//Layer 3 PCAP
internetv6.EnablePcapIpv6All(std::string ("wsnV6_PCAP_LINE7_ALT_400Bytes"));
//Layer 2 PCAP
lrWpanHelper.EnablePcap(std::string ("wsnL2_PCAP_LINE7_ALT_400Bytes"),lrwpanDevices, true);
