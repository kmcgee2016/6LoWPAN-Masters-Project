      ...
      if (m_errorModel != 0)
        {
          ...
          *interferenceAndNoise += *m_noise;
		  ...
		  //KMG: Tests without noise effects
		  per = 0;
          ...
          double rValue = m_random->GetValue ();//value between 0 and 1
          /* Add a noise scaling factor(if needed) */
          rValue = (rValue/1);
          if (rValue < per)
            {
              // The packet was destroyed, drop the packet after reception.
              m_currentRxPacket.second = true;
              NS_LOG_UNCOND("frame dropped! - Noise\n");
            }else{
                    NS_LOG_UNCOND("frame not dropped - Clean\n");
                }
        
          //NS_LOG_UNCOND("lr-wpan-phy.cc -> LrWpanPhy::CheckInterference - - - per value = " << per << ", random value = " << rValue);
        }
      else
        {
  ...
  if (currentRxParams == params)
    {
      ...
      if (!m_currentRxPacket.second)
      ...
      else
        {
          ...
          //KMG
          //NS_LOG_UNCOND("lr-wpan-phy.cc -> LrWpanPhy::EndRx - - Packet dropped on device " << m_device->GetAddress());
        }
