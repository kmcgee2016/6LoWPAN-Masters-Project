...
///////////////////////////////////////////////////////////////// KMG Functions /////////////////////////////////////////////////////////////
Mac16Address LrWpanMac::GetMeshParentAddress(void) const
{
	return m_MeshParentAddress;
}

void LrWpanMac::SetMeshParentAddress(Mac16Address addr){
	Mac16Address oldParentAddress = m_MeshParentAddress;
	m_MeshParentAddress = addr;
	//update the MAC array entries that had the old parent MAC as the next hop(best thing that can be done, given the scenario)
	for(uint8_t i = 0; i < 100; i++){
		if(m_nextMeshHopAddr[i] == oldParentAddress)m_nextMeshHopAddr[i] = addr;
	}
	//DYB! - replace the old parent entries(multiple if there are few neighbours)
	for(uint8_t j = 0; j < 6; j++){
		if(m_neighbourAddr[j] == oldParentAddress)m_neighbourAddr[j] = addr;
	}
}

Mac16Address LrWpanMac::GetNextHopForIndex(uint16_t macIndex)const
{
	return m_nextMeshHopAddr[macIndex]; //may be an issue here with type
}

void LrWpanMac::SetNextHopForIndex(Mac16Address nextHop, uint16_t macIndex)
{
	Mac16Address prevHopAtIndex = m_nextMeshHopAddr[macIndex];
	m_nextMeshHopAddr[macIndex] = nextHop; //may be an issue here with type
	
	//try and add this "next hop" address to the neighbour array:
	Mac16Address indexAddress = Mac16Address("ff:ff");//sentinal value
	uint8_t indexValue = -1;
	bool replaceOldHop = false;
	if(prevHopAtIndex != m_MeshParentAddress){
		//we are replacing an old entry with a new one => we need to replace the old entry in the neighbour list
			replaceOldHop = true;
	}
	while(indexAddress != m_MeshParentAddress){
		indexValue++;
		indexAddress = m_neighbourAddr[indexValue];
		if(replaceOldHop && indexAddress == prevHopAtIndex) break;//let's replace the old entry in the array
		if(indexAddress == nextHop) return; //it already exists in the list
	}
	//the index is now for the next empty element in the neighbour array
	if(indexValue != 5)m_neighbourAddr[indexValue] = nextHop; //but do not allow the overwrite of the final entry as it must be the parent address
}

void LrWpanMac::purgeNeighbourList(void)
{
	for(uint8_t i = 0; i < 6; i++){
		m_neighbourAddr[i] = m_MeshParentAddress;
	}
}
//FCNS. to populate the neighbour array:
//m_neighbourAddr
//TODO: Need a purge fcn. to clear the neighbour array and a fcn. to replace one neighbour with another - difficult to clear just one entry!
//-> replacement aadded in to LrWpanMac::SetNextHopForIndex
//-> purge fcn. wrote(simplistic ~ MEH!)
///////////////////////////////////////////////////////////////////// End ///////////////////////////////////////////////////////////////////
...
LrWpanMac::McpsDataRequest (McpsDataRequestParams params, Ptr<Packet> p)
{
  ...
  /*KMG Modifications go in here
   * if(m_forwardFrame == false) => locally generated frame
   * -> dstPanId = finalDstAddr
   * -> srcAddr = GetShortAddress()
   * -> dstAddr set to m_nextMeshHopAddr[uint16_t(finalDstAddr)]
   * -> If(dstAddr == 0xffff) => broadcast initial Tx
   * 	->  srcPanId = GetShortAddress()
   * 	->  dstAddr = 0xffff
   * 	->  srcAddr = GetShortAddress()
   * else => m_forwardFrame == true => re-transmit
   * -> If(dstAddr == 0xffff) => broadcast re-Tx
   * 	->  srcAddr = srcAddr
   * 	->  srcPanId = GetShortAddress()
   * 	->	dstAddr = 0xffff
   * 	Else => normal frame forwarding
   * 	->  dstAddr = m_nextMeshHopAddr[this->dstPanId]
   * 	->  srcAddr = srcAddr
   * 	->  ..
   * */
   uint8_t thisAddrParts[2];
   Mac16Address myAddress = GetShortAddress ();
   myAddress.CopyTo(thisAddrParts);
   uint16_t sixteenBitAddr = ((uint16_t)(thisAddrParts[1])) | ((thisAddrParts[0]) << 8);
   if(params.m_forwardFrame){
		if(params.m_dstAddr == Mac16Address("ff:ff")){
		   //this is a broadcast retransmission
		   //=> update srcPanId and Tx
		   //NS_LOG_UNCOND("Broadcast frame being forwarded, srcPanId: " << sixteenBitAddr << " and node address: " << GetShortAddress() << " source: " << params.m_srcAddr);
		   
		   macHdr.SetSrcAddrFields (sixteenBitAddr, params.m_srcAddr);
		   macHdr.SetDstAddrFields (uint16_t(0xffff), params.m_dstAddr);
		}else{
			//this is a standard frame retransmission
			//=> update dstAddr and Tx(but leave dstPanId as is!)
			//uint8_t dstAddrParts[2];
			//uint16_t dstAddrLong = params.m_dstPanId;
			//dstAddrParts[0] = uint8_t(dstAddrLong & 0xff);
			//dstAddrParts[1] = uint8_t((dstAddrLong >> 8) & 0xff);
			Mac16Address nextHopAddr = m_nextMeshHopAddr[params.m_dstPanId];
			//NS_LOG_UNCOND("Reg. frame forwarding to " << params.m_dstPanId << " via next hop " << nextHopAddr << ". Node= " << GetShortAddress());
			
			macHdr.SetSrcAddrFields(sixteenBitAddr, params.m_srcAddr);
			macHdr.SetDstAddrFields(params.m_dstPanId, nextHopAddr);
			macHdr.SetAckReq ();//try this out to see
		}
   }else{
	   //NS_LOG_UNCOND("Frame originating at " << GetShortAddress() << " with destination " << params.m_dstAddr);
	   if(params.m_dstAddr == Mac16Address("ff:ff")){
		   //this is a broadcast initial transmission
		   //=> set srcAddr, srcPanId
		   //NS_LOG_UNCOND("Broadcast frame originating at " << GetShortAddress());
		   macHdr.SetSrcAddrFields (sixteenBitAddr, myAddress);
		   macHdr.SetDstAddrFields (uint16_t(0xffff), params.m_dstAddr);
		}else{
			//this is a standard frame initial transmission
			//=> set the dstPanId, srcAddr, ect.
			macHdr.SetSrcAddrFields (sixteenBitAddr, myAddress);
			
			
			uint8_t finAddrParts[2];
			params.m_dstAddr.CopyTo(finAddrParts);
			uint16_t finAddrLong = ((uint16_t)(finAddrParts[1])) | ((finAddrParts[0]) << 8);
			Mac16Address nxtHopAddr = m_nextMeshHopAddr[finAddrLong];
			macHdr.SetDstAddrFields (finAddrLong, nxtHopAddr);
			//NS_LOG_UNCOND("Regular frame generated by " << GetShortAddress() << ", destination: " << params.m_dstAddr << " or " << finAddrLong << "... via next hop: " << nxtHopAddr);
		}
   }
   //////////////////////////////
  ...
  if (b0 == TX_OPTION_ACK)
    ...
    {
      //macHdr.SetNoAckReq ();// KMG: Commented this out for now
    }
  else
    {
  ...
  //KMG Modification: (Hard-)Stop for ACKs on broadcast messages:
  if(params.m_dstAddr == Mac16Address("ff:ff")) macHdr.SetNoAckReq (); //just to be sure
...
LrWpanMac::CheckQueue ()
{
  ...
  if (m_lrWpanMacState == MAC_IDLE && !m_txQueue.empty () && m_txPkt == 0 && !m_setMacState.IsRunning ())
    {
      txCount++;
      //NS_LOG_UNCOND("Time:, "<<Simulator::Now()<<", Address =, " << GetShortAddress() << ", TxQueueSize = ," << m_txQueue.size() << ", rxCount = , " << rxCount << ", TxCount =, " << txCount << ",(Tx)");
  ...
  else
    {
      ...
      if (m_macPromiscuousMode)
      ...
      else
        {
		  ...
		  //KMG Block unwanted broadcast messages:
		  if(acceptFrame && receivedMacHdr.GetType () == LrWpanMacHeader::LRWPAN_MAC_DATA && (params.m_dstAddr == Mac16Address("ff:ff"))){
			  //if this message came from a last hop that does not sit on the source path: block it
			  //params.m_srcPanId in this case is the last hop
			  uint8_t lastHopAddrParts[2];
			  receivedMacHdr.GetShortSrcAddr().CopyTo(lastHopAddrParts);
			  uint16_t lastHopAddrLong = uint16_t(lastHopAddrParts[0] << 8) | (lastHopAddrParts[1] & 0xff);
			  Mac16Address lastHopAddr = m_nextMeshHopAddr[lastHopAddrLong];
			  //NS_LOG_UNCOND("Last hop: " << lastHopAddr << " which is based off " << receivedMacHdr.GetShortSrcAddr() << " via " << receivedMacHdr.GetSrcPanId());
			  lastHopAddr.CopyTo(lastHopAddrParts);
			  lastHopAddrLong = uint16_t(lastHopAddrParts[0] << 8) | (lastHopAddrParts[1] & 0xff);
			  acceptFrame = (lastHopAddrLong == receivedMacHdr.GetSrcPanId() && receivedMacHdr.GetShortSrcAddr() != GetShortAddress());//is the last Tx node, the first hop to the true source?
			  //NS_LOG_UNCOND("lrWpanMac::PdDataIndication -> broadcast from long : " << lastHopAddrLong << " to " << GetShortAddress() << " originating at " << receivedMacHdr.GetShortSrcAddr() << ". Accepted = " << acceptFrame);
			}
          if (acceptFrame)
            {
          ...
          if (acceptFrame)
            {
				...
				/*
				 * KMG Modification: Need to stop Rx above L2 for forwarded frames - Done
				 * KMG Modification: Need to invoke the Re-Tx after any possible ACK is completed
				 * 
				 * */
				 m_macRxTrace (originalPkt); //not sure whether to block this in the event of regular forwarding
			  uint8_t thisDstAddrParts[2];
			  receivedMacHdr.GetShortDstAddr().CopyTo(thisDstAddrParts);
			  //uint16_t thisDstAddrLong = uint16_t(thisDstAddrParts[1] << 8) | (thisDstAddrParts[0] & 0xff);
			  uint16_t thisDstAddrLong = uint16_t(thisDstAddrParts[0] << 8) | (thisDstAddrParts[1] & 0xff);
			  bool frameForwardNeeded = false;
			  bool isMulticast = (receivedMacHdr.GetShortDstAddr() == Mac16Address("ff:ff"));
			  if(thisDstAddrLong != receivedMacHdr.GetDstPanId()){
				  frameForwardNeeded = true;
			    }
			  //NS_LOG_UNCOND("Frame accepted on node " << GetShortAddress() << ". Broadcast: " << isMulticast << ", frameForwardNeeded: " << frameForwardNeeded << ", thisDstAddrLong: " << thisDstAddrLong << ", receivedMacHdr.GetDstPanId(): " << receivedMacHdr.GetDstPanId());	
		      ...
		      /*
		       * KMG DATA broadcast Re-Tx code here, but need to screen for:
		       * -> dstAddr == 0xffff && m_nextMeshHopAddr[dstAddrLong] != Mac16srcPanId
		       * -> !receivedMacHdr.IsData ()
		       * 
		       * */
		      if (receivedMacHdr.IsData() && (frameForwardNeeded || isMulticast) && receivedMacHdr.GetShortSrcAddr() != GetShortAddress()){
					//lets do some forwarding, now that the ACK has/is being handled
					//**BUT FIRST-> Make a copy of "p", otherwise both Tx and Rx paths make reference to the same object!!! => L2 Header, ect. added before L3 reception!
					Ptr<Packet> outPkt = p->Copy ();
					McpsDataRequestParams outParams; // THIS MUST BE POPULATED FULLY & PROPERLY!
					outParams.m_msduHandle = 0;
					if(isMulticast){
						outParams.m_dstAddr = receivedMacHdr.GetShortDstAddr();
						outParams.m_dstAddrMode = SHORT_ADDR;
						outParams.m_dstPanId = uint16_t(0xffff);
						outParams.m_srcAddrMode = SHORT_ADDR;
						outParams.m_srcAddr = receivedMacHdr.GetShortSrcAddr();
						outParams.m_forwardFrame = true;
					}else{
						outParams.m_dstAddr = m_nextMeshHopAddr[receivedMacHdr.GetDstPanId()];
						outParams.m_dstAddrMode = SHORT_ADDR;
						outParams.m_dstPanId = receivedMacHdr.GetDstPanId();
						outParams.m_srcAddrMode = SHORT_ADDR;
						outParams.m_srcAddr = receivedMacHdr.GetShortSrcAddr();
						outParams.m_forwardFrame = true;	
					}
					LrWpanMac::McpsDataRequest (outParams, outPkt);
				  }
              if ((!frameForwardNeeded || isMulticast) && receivedMacHdr.IsData () && !m_mcpsDataIndicationCallback.IsNull ())
                {
                  // If it is a data frame, push it up the stack.
                  NS_LOG_DEBUG ("PdDataIndication():  Packet is for me; forwarding up");
                  //NS_LOG_UNCOND ("PdDataIndication():  Packet is for me; forwarding up");
                  m_mcpsDataIndicationCallback (params, p);
                }
                ...
                else if(frameForwardNeeded && !isMulticast)m_macRxDropTrace (originalPkt);//clean up
            }
          else
            {
              m_macRxDropTrace (originalPkt);
...
LrWpanMac::SendAck (uint8_t seqno)
{
  ...
  rxCount++;
                //NS_LOG_UNCOND("Time:, "<<Simulator::Now()<<", Address =, " << GetShortAddress() << ", TxQueueSize = ," << m_txQueue.size() << ", rxCount = , " << rxCount << ", TxCount =, " << txCount << ",(ACK)");
...
LrWpanMac::PrepareRetransmission (void)
{
  ...
  //KMG: Add the limit on CSMA-CA retries(main cause of errors in tests)
  if (m_retransmission >= m_macMaxFrameRetries /*|| m_numCsmacaRetry >= 5*/ )
    {
