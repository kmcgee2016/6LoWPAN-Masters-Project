...
LrWpanCsmaCa::LrWpanCsmaCa ()
{
  ...
  m_BLE = false;
  //KMG Modification:
  m_macMinBE = 3;//TRUE Value
  //m_macMaxBE = 10;
  m_macMaxBE = 5;//TRUE Value
  //m_macMinBE = 8;
  //m_macMaxBE = 13;
...
LrWpanCsmaCa::RandomBackoffDelay ()
{
  ...
  //KMG Modification: Increase the floor backoff time
  //backoffPeriod = (uint64_t)m_random->GetValue (0, upperBound+1); // num backoff periods
  backoffPeriod = (uint64_t)m_random->GetValue ((uint64_t)(m_macMinBE), upperBound+1); // num backoff periods
  ...
  if (IsUnSlottedCsmaCa ())
    {
      ...
      //NS_LOG_UNCOND ("Unslotted:  requesting CCA after backoff of " << randomBackoff.GetMicroSeconds () << " us. CSMA-CA Attempt: " << m_NB << ", m_BE = " << m_BE);
