...
struct McpsDataRequestParams
{
  ...
  //KMG Modification: Used to highlight forwarded Frames
  bool m_forwardFrame;
  //KMG Modification: stops retransmission to broadcast msg. source
...
public:
  ...
  ///////////////////////////////////////////////////////////////////////// KMG functions ///////////////////////////////////////////////////////
  //Mac16Address m_MeshParentAddress;
  Mac16Address GetMeshParentAddress(void) const;
  void SetMeshParentAddress(Mac16Address addr);
  
  //Mac16Address m_nextMeshHopAddr[32];
  Mac16Address GetNextHopForIndex(uint16_t macIndex)const;//may be an issue here with type
  void SetNextHopForIndex(Mac16Address nextHop, uint16_t macIndex);
  
  void purgeNeighbourList(void);
  ///////////////////////////////////////////////////////////////////////// END /////////////////////////////////////////////////////////////////
protected:
...
private:
  ...
  //KMG Modification: Give final MAC address as uint16_t for quick comparison upon Tx
  uint16_t m_finalMacAddress; //Is this device's MAC address but in uint16_t form - never modified directly
  //KMG Modification: Set the mesh parent node(uint16_t)
  Mac16Address m_MeshParentAddress;
  //KMG Modification: 32-entry routing table[pre configured]
  Mac16Address m_nextMeshHopAddr[100]; //maximum of 32 nodes for now with final Dst. MAC addresses= 0->32
  // -> This array maybe should default to each entry being the parent address?
  Mac16Address m_neighbourAddr[6]; //maximum of 6 neighbouring nodes(prepopulated with the parent node)
  // -> This needs to be cleverly populated, ie. each new neighbour needs to be added to the list up to index[4]
};
